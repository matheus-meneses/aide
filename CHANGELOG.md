...
- **Mark a task done from the feed** — each item card in the web feed now has a
  "Mark done" action that closes the item locally without waiting for the source
  to drop it. Done items move to a new terminal `status='done'` state that is
  preserved across future scrapes (the `Upsert` ON CONFLICT clause no longer
  resets a done item back to `open`), so a manually closed item never reappears.
  All open queries already filter by `status='open'`, so done items disappear from
  the feed and open counts for free; the count badge refreshes immediately instead
  of waiting for the 60s poll. Backed by a new `POST /api/items/done` endpoint.
  Done state is local to aide — there is no write-back to the source system. (#80)

- **AI-synthesized daily briefings** — when a model is configured, the scheduled
  daily briefing is now generated by the LLM from the same guardrailed context the
  assistant uses, producing a concise, prioritized summary (what's urgent, what's
  happening today, what else is open) instead of a flat count. The synthesis runs
  under the immutable safety/precedence + core-rules layer with all scraped data kept
  inside the untrusted-data fence, so a crafted item can't hijack the briefing. If the agent is
  unavailable for any reason — no model configured, a context/build error, an LLM
  failure, or empty output — aide falls back to the deterministic template, so a
  briefing always goes out and the offline path never hits the network. Scheduling
  (`agent.briefing_times`) and notification channels are unchanged. (#63)
...
[Unreleased]: https://github.com/matheus-meneses/aide/compare/v0.4.0...HEAD
[0.4.0]: https://github.com/matheus-meneses/aide/compare/v0.3.3...v0.4.0
[0.3.3]: https://github.com/matheus-meneses/aide/compare/v0.3.2...v0.3.3

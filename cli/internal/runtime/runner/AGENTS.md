...
**macOS sandbox + trust export.** Plugins run under a `sandbox-exec` profile (`(deny default)`, no Mach) that blocks the Mach/`trustd` calls macOS needs to evaluate trust natively — so `truststore` cannot verify against the system store from inside the sandbox (symptom: `unable to get local issuer certificate`). To keep the sandbox tight, on macOS the trust store is exported to a cached PEM (`trust.SystemBundle()` in `internal/runtime/trust` → `security find-certificate -a -p` over the system/admin/login keychains → `~/.aide/cache/system-trust.pem`, 6h TTL) and injected as `ca_bundle` whenever verification is on and no explicit bundle is set. OpenSSL then verifies via file-read, which the sandbox allows. The exporter lives in its own package (not `runner`) so the plugin installer can reuse it to feed `NODE_EXTRA_CA_CERTS` for Playwright browser downloads. Linux needs no export (its sandbox reads `/etc/ssl` directly), so `trust.SystemBundle()` is a no-op there.

        ...
        <div className="rounded-lg border bg-card overflow-hidden">
            <div className="px-3 py-2 border-b bg-accent/30 flex items-center justify-between">
                ...
                <div className="flex gap-1.5">
                    {Object.entries(grouped).map(([source, items]) => (
                        <span key={source} className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground">
              {source}: {items.length}
            </span>
                    ))}
                </div>
            ...
            <div className="divide-y">
                {Object.entries(visibleGrouped).map(([source, items]) => (
                    <div key={source}>
                        <div
                            className="px-3 py-1.5 bg-accent/20 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
                            {source}
                        </div>
                        {items.map((item, i) => (
                            <div

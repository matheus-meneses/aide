...
function TreeNode({member, tree, depth}: { member: Member; tree: Map<number | null, Member[]>; depth: number }) {
        ...
        <div>
            ...
            >
                ...
                <div className="flex flex-col min-w-0">
                    <span className="text-sm font-medium leading-tight truncate">{member.name}
                        {member.registration && (
                            <span className="ml-1.5 text-xs text-muted-foreground font-normal">({member.registration})</span>
                        )}
                    </span>
                    {member.role && (
                        <span className="text-xs text-muted-foreground leading-tight truncate">{member.role}</span>
                    )}
                </div>
        ...
        <div className="rounded-lg border bg-card overflow-hidden">
            ...
            {mode === 'tree' ? (
                <div className="py-1">
                    ...
                    {members.filter(m => m.manager_id == null && !roots.find(r => r.id === m.id)).map(m => (

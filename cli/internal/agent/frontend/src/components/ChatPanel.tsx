...
export function ChatPanel({pendingEvent, onEventConsumed, onChatMessage}: Props) {
    const {messages, send, isStreaming, cancel, injectMessage, clearMessages, appendAssistantFromSSE, retry} = useChatStream('/api/chat')
    ...
    useEffect(() => {
        ...
        const existsInChat = matchText.length > 0 && messagesRef.current.some(m =>
            m.role === 'assistant' && m.content.includes(matchText)
        )
        ...
        <div className="flex flex-col h-full">
            ...
            >
                <div className="max-w-3xl mx-auto">
                    {messages.length === 0 && (
                        <div
                            className="flex flex-col items-center justify-center h-full min-h-[60vh] text-muted-foreground">
                            <div className="text-2xl font-light mb-2">{userName ? `Hi ${userName}` : 'Aide'}</div>
                            <div
                                className="text-sm mb-6">{userName ? 'How can I help you today?' : 'Your personal work assistant. Ask about tasks, meetings, or items.'}</div>

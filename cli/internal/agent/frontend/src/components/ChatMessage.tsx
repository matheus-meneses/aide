...
function RichContent({format, data, content, onSuggestionClick}: {
    format?: string;
    data?: any;
    content: string;
    onSuggestionClick?: (text: string) => void
}) {
    if (format === 'schedule' && data) return <ScheduleView data={data}/>
    if (format === 'items' && data) return <ItemsView data={data}/>
    if (format === 'status' && data) return <StatusView data={data}/>
    if (format === 'memory') return <MemoryView data={data} text={content}/>
    if (format === 'stats' && data) return <StatsView data={data}/>
    if (format === 'team' && data) return <TeamView data={data}/>
    if (format === 'scrape' && data) {
        return (
    ...
    if (format === 'text') {
        return <pre className="text-sm whitespace-pre-wrap font-mono bg-muted rounded p-3">{content}</pre>
    }
    return <MarkdownRenderer content={content} onSuggestionClick={onSuggestionClick}/>
}

export function ChatMessage({role, content, timestamp, isStreaming, isError, format, data, onSuggestionClick, onRetry}: Props) {
                ...
                <div
                    className={cn(
                        ...
                        isUser
                            ...
                            : format && format !== 'markdown' ? '' : 'bg-card border'
                    )}
                >
                    {isUser ? (
                        ...
                        <div>
                            <RichContent format={format} data={data} content={content}
                    ...
                    {isStreaming && !content && (
                        <div className="flex gap-1 py-1">
                            <span
                                className="w-1.5 h-1.5 bg-muted-foreground rounded-full animate-bounce [animation-delay:0ms]"/>
                            <span
                                className="w-1.5 h-1.5 bg-muted-foreground rounded-full animate-bounce [animation-delay:150ms]"/>
                            <span
                                className="w-1.5 h-1.5 bg-muted-foreground rounded-full animate-bounce [animation-delay:300ms]"/>
                        </div>
                    )}
                </div>
                <span
                    className="text-[10px] text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity px-1">
          {timeAgo(timestamp)}
        </span>
            </div>
        </div>

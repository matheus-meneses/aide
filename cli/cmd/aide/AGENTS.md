...
| `agent.go` | `aide agent start` (headless foreground loop) / `aide agent status` / `aide agent ask` / `aide agent schedule` / `aide agent context` (view/set/clear user + per-source context) |
...
- Registry TLS: the registry HTTPS client (`internal/runtime/plugin/registry.go`) verifies certificates by default. For a self-hosted registry served by an internal CA, set `AIDE_REGISTRY_CA_BUNDLE` to a PEM path; those roots are appended to the system pool. There is no insecure/skip-verify switch for the registry.
